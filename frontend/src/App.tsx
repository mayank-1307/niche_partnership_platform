...
function formatRecentRound(item: unknown): string {
    ...
    if (trimmed.startsWith("{")) {
      try {
        ...
        if (parsed && typeof parsed === "object") {
          return formatRecentRound(parsed);
        }
      } catch {
        return trimmed;
      }
  ...
  const parts: string[] = [];
  const roundType = typeof round.round_type === "string" ? round.round_type.trim() : "";
  if (roundType) parts.push(roundType);

  const amount = formatUsd(round.amount_usd);
  if (amount !== "-") parts.push(amount);

  const year = round.year;
  if (typeof year === "number" || (typeof year === "string" && year.trim())) {
...
function formatRecentRounds(value: unknown): string[] {
  if (!Array.isArray(value)) return [];
  return value.map(formatRecentRound).filter((item) => item.length > 0);
}

function formatKeyLeader(item: unknown): string {
  if (typeof item === "string") {
    return item.trim();
  }
  ...
  const name = typeof leader.name === "string" ? leader.name.trim() : "";
  const title = typeof leader.title === "string" ? leader.title.trim() : "";
  const background = typeof leader.background === "string" ? leader.background.trim() : "";

  if (!name) return "";

  const headline = title ? `${name} (${title})` : name;
  return background ? `${headline} — ${background}` : headline;
}

function formatKeyLeaders(value: unknown): string[] {
  if (!Array.isArray(value)) return [];
  return value.map(formatKeyLeader).filter((item) => item.length > 0);
}

function getEnterpriseSectionFacts(enterprise: Record<string, unknown>, legacyKey: string, subPartKey: string): Record<string, unknown> {
  const legacySection = asRecord(enterprise[legacyKey]);
  if (Object.keys(legacySection).length > 0) {
    return legacySection;
  }

  const subParts = asRecord(enterprise.sub_parts);
  const subPart = asRecord(subParts[subPartKey]);
  return asRecord(subPart.facts);
}
...
export default function App() {
  ...
  const inputPlaceholder = isAnalyzeMode
    ? "Please enter the partner company website (Eg: https://company.com)"
    : "Please enter the partner company name (Eg: Company Name)";
  ...
  const loadRecentAnalysis = async (profileId: number) => {
    ...
    try {
      ...
      if (typeof (structured as any).website === "string" && (structured as any).website.trim()) {
        setDomain((structured as any).website.trim());
      } else if (profile.company_name) {
        setDomain(profile.company_name.trim());
      }
      setInputLocked(true);
  ...
  const funding = getEnterpriseSectionFacts(enterprise, "funding", "institutional_funding");
  const leadership = getEnterpriseSectionFacts(enterprise, "leadership", "proven_leadership_team");
  ...
  const rounds = formatRecentRounds(funding.recent_rounds);
  const leaders = formatKeyLeaders(leadership.key_leaders);
      ...
      <div className="mx-auto max-w-7xl px-4 pb-8 pt-24 md:px-8">
        ...
        <section className="glass mb-8 rounded-2xl p-4 md:p-6">
          <div className="mb-4 flex flex-wrap gap-4" role="radiogroup" aria-label="Analysis input mode">
            <label
              className={`inline-flex items-center gap-2 rounded-lg border border-white/10 bg-black/25 px-4 py-2 text-sm text-slate-200 transition has-[:checked]:border-cyan/50 has-[:checked]:bg-cyan/10 ${isInputFrozen ? "cursor-not-allowed opacity-60" : "cursor-pointer hover:bg-white/5"
                }`}
            >
              <input
                type="radio"
